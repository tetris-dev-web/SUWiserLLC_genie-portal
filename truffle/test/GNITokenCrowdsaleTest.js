...
contract('GNITokenCrowdsale', async (_accounts) => {
  ...
  describe('removeVotesFromProject_', async () => {
    before(async () => {
      ...
      console.log(projStub1.address)
      await mockGTC._removeVotesFromProject_(accounts[2], projStub1.address, 20000000);
    })
  ...
  describe('removeVotesFromProjectByAccount', async () => {
    describe('when the project is not open', async () => {
      before(async () => {
        ...
        await mockGTC.removeVotesFromProjectByAccount(accounts[2], projStub1.address);
      })
      ...
      it('adds all votes that the account dedicated to the project to the accounts vote credit', async () => {
        ...
        assert.equal(secondAddress, projStub1.address, 'votes not removed from the correct project');
  ...
  //
  //     await mockGTC.voteForProject(0, 20000000, {from: accounts[2]});
  //   })
  //
  //   after(async () => {
  //     await stubUtil.resetMethod(projStub1, 'vote');
