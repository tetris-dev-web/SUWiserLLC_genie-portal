    ...
    describe("when the sender is the owner", async () => {
      describe("when co amenments can still be added", async () => {
        ...
        it("increments totalCoAmendments by 1", async () => {
          ...
          assert.equal(Number(totalCoAmendmentsT2), Number(totalCoAmendmentsT1) + 1, "totalCoAmendments should increment by 1");
        })
      ...
      describe("when co amendments can not longer be added", async () => {
        ...
        it("reverts", async () => {
          await exceptions.catchRevert(amendment.addCoAmendment(accounts[1], {from: accounts[0]}));
        })
      })
    })
