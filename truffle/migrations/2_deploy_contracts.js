...
module.exports = function (deployer, network, accounts) {
  ...
  return (
    deployer
      .then(() => {
        return deployer.deploy(ProjectLeaderTracker);
      })
      .then(() => {
      ...
      .then(() => {
        return deployer.deploy(InactiveToken, VotingToken.address, ActiveToken.address);
      })
      .then(() => {
        return deployer.deploy(Dividends, ActiveToken.address);
      })
      .then(() => {
        return deployer.deploy(Reimbursements, InactiveToken.address);
      })
      .then(() => {
        return deployer.deploy(Activation, InactiveToken.address, ProjectLeaderTracker.address);
      })
      ...
      .then(() => {
        return deployer.deploy(TokenPurchaseHelper);
      })
      .then(() => {
        return network === 'ropsten'
          ? SeedableCrowdsale.at(SeedableCrowdsale.address)
          : GNITokenCrowdsale.at(GNITokenCrowdsale.address);
      })
