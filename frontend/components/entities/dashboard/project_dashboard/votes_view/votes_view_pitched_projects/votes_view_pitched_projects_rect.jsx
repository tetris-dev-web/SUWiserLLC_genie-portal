...
const VotesViewPitchedProjectsRect = (props) => {
  const [state, setState] = React.useState({
    showHoverEffect: false,
  });

  let outlineWidth = 2;
  let votesBreakdownRectHeight = 30;
  let projectGroup;
  ...
  const handleHover = (boolean) => {
    ...
    return () => {
      if (!selectedProject) {
        setState({ showHoverEffect: boolean });
      }
    };
  };

  const handleClick = (e) => {
    const { selectedProject, toggleSelectedProject, project, voteShiftTool } = props;
    if (!selectedProject) {
      document.addEventListener("click", handleClick, false);
      toggleSelectedProject(project);
      if (!state.showHoverEffect) setState({ showHoverEffect: true });
    } else {
      if (projectGroup.contains(e.target) || voteShiftTool.contains(e.target)) return;
      document.removeEventListener("click", handleClick, false);
      toggleSelectedProject(null);
      if (state.showHoverEffect) setState({ showHoverEffect: false });
    }
  };

  const { selectedProject, SVGWidth, project, circleScale, transform } = props;
  const {
    fill,
    marginWidth,
    projectStartX,
    projectWidth,
    projectValutionHeight,
    projectValutionStartY,
    projectCapitalRequiredHeight,
    projectCapitalRequiredStartY,
    projectRectCenter,
    capitalRequired,
    valuation,
    voteShare,
  ...
  const HoverEffects = () => (
    <g className="votes-view-onhover-group">
      ...
      <g className="votes-view-onhover-text-group">
        ...
        <line
          x1={projectRectCenter}
          y1={projectValutionStartY}
          x2={2000}
          y2={projectValutionStartY}
          stroke={fill}
        ...
        <line
          x1={projectRectCenter}
          y1={projectCapitalRequiredStartY}
          x2={2000}
          y2={projectCapitalRequiredStartY}
          stroke={fill}
          strokeDasharray="5,20"
          opacity={0.5}
        ></line>
      ...
      <g className="votes-view-onhover-project-outlines-group">
        ...
        ></rect>
        <rect
          x={projectStartX + projectWidth - outlineWidth}
          y={projectValutionStartY}
          width={outlineWidth}
  ...
  return (
    <g
      className="votes-view-project-group"
      transform={transform}
      onClick={handleClick}
      ref={(node) => (projectGroup = node)}
    >
      <rect
        width={projectWidth}
        height={projectValutionHeight}
      ...
      <text
        className="votes-view-percentage-breakdown"
        x={projectRectCenter}
      ...
      {state.showHoverEffect && <HoverEffects />}
    </g>
  );
};
