...
const VotesViewCapitalRaised = (props) => {
  const {
    SVGYScale,
    SVGHeightScale,
    SVGTimeXScale,
  ...
  console.log("VotesViewCapitalRaised.PROP=", props);
  ...
  const Lines = deployedProjects.map((project, idx) => {
    return (
      <ProjectCrossLine
        key={idx}
        x1={-Math.abs(SVGTimeXScale((startTime + endTime) / 2) / 0.8)}
        y1={SVGYScale(project.capital)}
        x2={SVGTimeXScale(project.activationTime)}
        y2={SVGYScale(project.capital)}
        opacity={opacityVar}
  ...
  const CapitalBeingRaisedRect = (
    <VotesViewCapitalRaisedRect
      x="0"
      y={yOfCapitalBeingRaisedRect}
      ...
      textToDisplay={() => (
        <g>
          <text
            ...
            x="85%"
            y={yOfCapitalBeingRaisedRect + heightOfCapitalBeingRaisedRect / 2}
          >
  ...
  const capRaisedAmounts = deployedProjects.map((project, idx) => {
    const capitalRequired =
      project.capitalRequired > 0
        ? "$ " + Number(project.capitalRequired / 1000.0).toLocaleString() + " k"
        : "";
    return (
      <text
        className="votes-view-capital-raised-text"
        ...
        y={
          idx
            ? SVGYScale((project.capital + deployedProjects[idx - 1].capital) / 2)
            : (SVGYScale(project.capital) +
                yOfCapitalRaisedPrevRect +
                heightOfCapitalRaisedPrevRect) /
              2
        }
      >
  ...
  const CapitalRaisedPrevRect = (
    <VotesViewCapitalRaisedRect
      x="0"
      y={yOfCapitalRaisedPrevRect}
      ...
      textToDisplay={() => (
        <g>
          <text
            className="votes-view-capital-raised-text"
            x="85%"
            y={yOfCapitalRaisedPrevRect + heightOfCapitalRaisedPrevRect / 2}
          >
