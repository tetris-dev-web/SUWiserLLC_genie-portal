...
const DoomsDayDetail = (props) => {
  // const [lineData, setLineData] = useState({lineChartData: [], lines:[]});
  const [lineChartData, setLineChartData] = useState([]);
  const [lines, setLines] = useState([]);

  const startGraph = () => {
    const graphBase = d3
      .select("#doomsday-graph")
      .append("svg")
      .attr("width", width)
      .attr("height", height);
  ...
  useEffect(() => {
      ...
      projects: [
        {
          ...
          end_date: new Date("6-12-2019"),
          tokens: 12,
        },
        {
          project: "Ryans Rooftop",
          date: new Date("09-20-2018"),
          end_date: new Date("5-17-2019"),
          tokens: 18,
        },
        {
          project: "Liams loung",
          date: new Date("12-01-2018"),
          end_date: new Date("7-28-2019"),
          tokens: 9,
        },
        {
          project: "Matts mansion",
          date: new Date("09-20-2018"),
    ...
    const data = formatData(dataPassedIn.projects);
      ...
      (() => {
        ...
        for (let i = 0; i < dataPassedIn.projects.length; i++) {
          ...
          let tokenDate = new Date(
            currProject.end_date.getTime() + 6 * currProject.tokens * 86400000,
          );
          arr.push([
    ...
    const rectData = dataPassToRect.map((d) => {
      ...
      return { x1, x2, y1, width, height, project, tokens };
    });
    ...
    setLineChartData((c) => lineChartData);
    ...
    graph
      ...
      .data([
        {
          x: 0,
          y: yScale(updatedData[1][0].project) - 10,
          text: "projects",
          fill: "#1F6D6C",
          font_size: "15px",
        },
        {
          x: xScale(todayDate) - 17,
          y: height - 25,
          text: todayDateText,
          fill: "#DEDBCF",
          font_size: "12px",
        ...
        {
          x: xScale(sleepDate) - 30,
          y: height - 25,
          text: "portal sleep date",
          fill: "#DEDBCF",
          font_size: "12px",
        },
        {
          x: xScale(sleepDate) - 18,
          y: height - 5,
          text: updatedSleepDate,
          fill: "#DEDBCF",
          font_size: "12px",
        },
        {
          x: xScale(sleepDate) + 30,
          y: yScale(updatedData[1][0].project) - 30,
          text: "50k dedicated",
          fill: "#1F6D6C",
          font_size: "15px",
        },
